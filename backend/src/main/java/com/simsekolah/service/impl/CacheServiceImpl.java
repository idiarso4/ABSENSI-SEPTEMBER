...
public class CacheServiceImpl implements CacheService {
    ...
    public Long getCacheSize(String cacheName) {
        if (redisTemplate == null) {
            logger.debug("Redis not available, returning 0 for cache size");
            return 0L;
        }
    ...
    public Set<String> getCacheKeys(String cacheName) {
        if (redisTemplate == null) {
            logger.debug("Redis not available, returning empty set for cache keys");
            return new HashSet<>();
        }
    ...
    public Long getCacheEntryTTL(String cacheName, String key) {
        if (redisTemplate == null) {
            logger.debug("Redis not available, returning -1 for TTL");
            return -1L;
        }
    ...
    public Boolean testCacheConnectivity() {
        if (redisTemplate == null) {
            logger.debug("Redis not available, connectivity test failed");
            return false;
        }
