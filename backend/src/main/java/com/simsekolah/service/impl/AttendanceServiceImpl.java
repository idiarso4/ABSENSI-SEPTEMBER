...
import com.simsekolah.dto.response.ClassAttendanceStatsResponse;
import com.simsekolah.dto.response.AttendanceHistoryStatResponse;
import com.simsekolah.dto.response.AttendanceHistoryResponse;
...
public class AttendanceServiceImpl implements AttendanceService {
    ...
    public AttendanceResponse updateAttendance(Long attendanceId, UpdateAttendanceRequest request) {
        ...
        // Store the old state for history tracking
        AttendanceStatus oldStatus = attendance.getStatus();
        String oldKeterangan = attendance.getKeterangan();
        ...
        createAttendanceHistory(attendance, oldStatus, oldKeterangan);
    ...
    public AttendanceService.BulkAttendanceResult bulkRecordAttendance(BulkAttendanceRequest request) {
        ...
        for (var studentAttendance : request.getStudentAttendances()) {
            try {
                ...
                attendanceRequest.setStatus(AttendanceStatus.valueOf(studentAttendance.getStatus().name()));
    ...
    @Override
    @Transactional(readOnly = true)
    public List<StudentMiniResponse> getStudentsPendingAttendance(Long teachingActivityId) {
        log.info("Fetching students pending attendance for teaching activity ID: {}", teachingActivityId);

        // 1. Find the teaching activity and its associated classroom
        TeachingActivity teachingActivity = findTeachingActivityById(teachingActivityId);
        ClassRoom classRoom = teachingActivity.getClassRoom();
        if (classRoom == null) {
            log.warn("Teaching activity {} has no associated classroom.", teachingActivityId);
            return Collections.emptyList();
        }

        // 2. Get all students in that classroom
        List<Student> allStudentsInClass = studentRepository.findByClassRoomId(classRoom.getId());

        // 3. Get all students who already have an attendance record for this activity
        List<Attendance> existingAttendances = attendanceRepository.findByTeachingActivity(teachingActivity);
        Set<Long> studentsWithAttendance = existingAttendances.stream()
                .map(attendance -> attendance.getStudent().getId())
                .collect(Collectors.toSet());

        // 4. Filter out students who already have attendance recorded
        return allStudentsInClass.stream()
                .filter(student -> !studentsWithAttendance.contains(student.getId()))
                .map(student -> StudentMiniResponse.builder()
                        .id(student.getId())
                        .nis(student.getNis())
                        .name(student.getNamaLengkap()).build())
                .collect(Collectors.toList());
    }
    ...
    public List<AttendanceHistoryResponse> getAttendanceHistory(Long attendanceId) {
        log.info("Fetching history for attendance ID: {}", attendanceId);
        if (!attendanceRepository.existsById(attendanceId)) {
            throw new ResourceNotFoundException("Attendance not found with ID: " + attendanceId);
        }

        List<AttendanceHistory> historyRecords = attendanceHistoryRepository.findByAttendanceIdOrderByUpdatedAtDesc(attendanceId);
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<AttendanceHistoryResponse> getHistoryByTeacher(Long teacherId, Pageable pageable) {
        log.info("Fetching attendance history for teacher ID: {}", teacherId);
        Page<AttendanceHistory> historyPage = attendanceHistoryRepository.findByUpdatedByIdOrderByUpdatedAtDesc(teacherId, pageable);
        return historyPage.map(AttendanceHistoryResponse::from);
    }

    @Override
    @Transactional(readOnly = true)
    public Page<AttendanceHistoryStatResponse> getAttendanceHistoryStats(Pageable pageable) {
        log.info("Fetching attendance history statistics.");
        return attendanceHistoryRepository.getChangeCountPerTeacher(pageable);
    }

    @Override
    @Transactional
    public AttendanceResponse revertAttendanceFromHistory(Long historyId) {
        log.info("Reverting attendance from history record ID: {}", historyId);

        // 1. Find the history record
        AttendanceHistory historyRecord = attendanceHistoryRepository.findById(historyId)
                .orElseThrow(() -> new ResourceNotFoundException("Attendance history not found with ID: " + historyId));

        // 2. Get the associated attendance record
        Attendance attendanceToRevert = historyRecord.getAttendance();

        // 3. Apply the old values back to the attendance record
        attendanceToRevert.setStatus(historyRecord.getOldStatus());
        attendanceToRevert.setKeterangan(historyRecord.getOldKeterangan());
        attendanceToRevert.setUpdatedAt(LocalDateTime.now());

        // 4. Save the reverted attendance record (this will not create a new history entry)
        Attendance revertedAttendance = attendanceRepository.save(attendanceToRevert);
        log.info("Successfully reverted attendance ID: {} from history ID: {}", revertedAttendance.getId(), historyId);
        return AttendanceResponse.from(revertedAttendance);
    }

    @Override
    @Transactional(readOnly = true)
    public ClassAttendanceStatsResponse getAttendanceStatsByClassRoom(Long classRoomId, int year, int month) {
        log.info("Fetching attendance stats for classRoomId: {}, year: {}, month: {}", classRoomId, year, month);

        ClassRoom classRoom = classRoomRepository.findById(classRoomId)
                .orElseThrow(() -> new ResourceNotFoundException("ClassRoom not found with ID: " + classRoomId));

        long totalStudents = studentRepository.countByClassRoomId(classRoomId);
        long teachingDays = teachingActivityRepository.countDistinctTeachingDaysByClassRoomAndMonth(classRoomId, year, month);

        List<Object[]> statsResult = attendanceRepository.getAttendanceStatsByClassRoomAndMonth(classRoomId, year, month);
        ...
        long totalPossibleAttendances = totalStudents * teachingDays;
        double presenceRate = (totalPossibleAttendances > 0)
                ? (double) presentCount * 100.0 / totalPossibleAttendances
                : 0.0;

        return ClassAttendanceStatsResponse.builder()
                .classRoomId(classRoomId)
                .classRoomName(classRoom.getName())
                .period(String.format("%d-%02d", year, month))
                .totalStudents(totalStudents)
                .totalTeachingDaysInMonth(teachingDays)
    ...
    private void createAttendanceHistory(Attendance attendance, AttendanceStatus oldStatus, String oldKeterangan) {
        // Only create a history record if something actually changed.
        if (Objects.equals(oldStatus, attendance.getStatus()) && Objects.equals(oldKeterangan, attendance.getKeterangan())) {
            return;
        }
        ...
        AttendanceHistory history = AttendanceHistory.builder()
                ...
                .oldKeterangan(oldKeterangan)
                .newKeterangan(attendance.getKeterangan())
                .updatedBy(currentUser)
                .updatedAt(LocalDateTime.now())
                .build();
        attendanceHistoryRepository.save(history);
        log.info("Created history record for attendance ID: {}", attendance.getId());
    }
}
